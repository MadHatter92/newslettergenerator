...
class AINewsletterTasks():
	def fetch_news_task(self, agent):
		return Task(
			...
			expected_output = """A list of top AI news story titles, URLs,
			 and ad brief summary for each story from the search results.
			 Example Output:
			 [
			 {
	...
	def analyze_news_task(self, agent, context):
		return Task(
			...
			async_execution = True,
			context = context,
			expected_output = """A markdown-formatted analysis for each
			news story, including a rundown, detailed bull and a "Why it matters" 
			...
			**Why it matters:** While AI-related ads have been rampant over
			 the last year, its Super Bowl presentation is important bcause...
			 """
			 )
	...
	def compile_newsletter_task(self, agent, context, callback_function):
		return Task(
			...
			agent = agent,
			context = context,
			expected_output = """A complete newsletter in markdown format,
			with a consistent style and layout.
			Example output:
			'# Top stories in AI today:\\n\\n
			- AI takes spotlight in Super Bowl commercials'\\n
			- Altman seeks TRILLIONs for global AI chip initiative\\n\\n

			## AI takes spotlight in Super Blow commercials \\n\\n
			**The Rundown: ** AI made a splash in this year\'s Super Bowl commercials...\\n\\n
			**The details:**...\\n\\n
			**Why it matters:**\\n\\n
			## Altman seeks TRILLIONS for global AI chip initiative\\n\\n
			**The Rundown: ** Open AI CEO Sam Altman is reportedly trying to raise Trillions...\\n\\n
			**The details:**...\\n\\n
			**Why it matters:**\\n\\n
			""",
